...
w_p = 0.9638303571428571
w_n = 0.03616964285714286
normalize = transforms.Normalize(
mean=[0.07038508222520161],
std=[0.1707208474911738])
...
#     url = f'/scratch2/jcahoon/argml_data/Gower_50kb/tarball/50kb_{{1..19}}.tar'
    url = f'/project/lgarcia1_998/jcahoon/Gower_50kb/50kb_{{1..19}}.tar'
...
#     url =  "/scratch2/jcahoon/argml_data/Gower_50kb/tarball/50kb_0.tar"
    url =  f'/project/lgarcia1_998/jcahoon/Gower_50kb/50kb_0.tar'
    dataset = (
        wds.WebDataset(url)
        .decode("l")
    #     .rename(image="input", info="output")
...
# ofile.close()
ofile = open(f'pr_curve/Gower_{model_name}_50k_{level}_infer_y_out.npy', "wb")
pickle.dump(torch.sigmoid(y_out), ofile)
ofile.close()

ofile = open(f'pr_curve/Gower_{model_name}_50k_{level}_infer_y_test.npy', "wb")
pickle.dump(y_test, ofile)
ofile.close()
torch.save(model.state_dict(), f'trained_models/Gower_{model_name}_50k_{level}_infer.model')
